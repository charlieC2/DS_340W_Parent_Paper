...
# Random forest
D1_forest <- randomForest(target ~ ., data = D1_train, importance = TRUE, proximity = TRUE)
...
```{r}
# Decision tree
confusion_tree <- confusionMatrix(data = as.factor(D1_predict_tree), reference = as.factor(target$target[!sample]))
confusion_tree

# LGBM
confusion_lgbm <- confusionMatrix(data = as.factor(D1_predict_lgbm), reference = as.factor(target$target[!sample]))
confusion_lgbm
```
